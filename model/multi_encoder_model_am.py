'''
Replace the decoder with AM decoder, to run experiments with AM.
'''
...
class MEAM_AMDECODER(nn.Module):
    ...
    def forward(self, x_all: torch.Tensor, dt_graph: torch.Tensor=None, decode_type='sample'):
        ...
        glimpse_keys = self.glimpseK(node_embeddings)
        glimpse_values = self.glimpseV(node_embeddings)
        ...
        des_embedding, cur_embedding = self.des_holder.expand(EB, -1), self.cur_holder.expand(EB, -1)
        graph_embedding = node_embeddings.mean(1)  # EB * hidden
        route_record = []
        probs_record = []
        first_prob = None
        for step in range(n):
            # des and cur can exchange!
            h = torch.cat([graph_embedding, des_embedding, cur_embedding], 1).unsqueeze(1)  # EB * 1 * 3hidden
    ...
    def train_self(self, problem_size=50, batch_size=512, epochs=100, steps_per_epoch=2500, lr=1e-4, \
        ...
        save_dir = os.path.join('save_am_decoder', 'tsp_{}'.format(problem_size), train_name)  # save/tsp_50/default
